...
class TestBuildStrategyInstructions:
    def test_tiny_bucket_guidance(self):
        result = build_strategy_instructions([], {}, "tiny", 100_000, 500_000)
        assert "lightweight" in result.lower()
        assert "attention" in result.lower()

    def test_small_bucket_guidance(self):
        result = build_strategy_instructions([], {}, "small", 500_000, 2_000_000)
        assert "small" in result.lower()

    def test_medium_bucket_guidance(self):
        result = build_strategy_instructions([], {}, "medium", 10_000_000, 50_000_000)
        assert "transformer" in result.lower()

    def test_large_bucket_guidance(self):
        result = build_strategy_instructions([], {}, "large", 50_000_000, 125_000_000)
        assert "transformer" in result.lower()
    ...
    def test_with_frontier_shows_best_crps(self):
        ...
        result = build_strategy_instructions(frontier, {}, "small", 500_000, 2_000_000)
    ...
    def test_includes_bucket_history(self):
        ...
        result = build_strategy_instructions([], state, "small", 500_000, 2_000_000)
