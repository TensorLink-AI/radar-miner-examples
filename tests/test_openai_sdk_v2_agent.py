...
# ── Module loading ────────────────────────────────────────────────
# Add the repo root so ``agents.openai_sdk_v2`` resolves as a package.
...
# openai_sdk package has its own ``agents.openai_sdk_v2.tools`` and that
...
def _reset_llm_client_cache() -> None:
    ...
    from agents.openai_sdk_v2 import llm_client
...
def _reset_core_resolution() -> None:
    ...
    captured it) so the next ``from agents.openai_sdk_v2 import agent``
    ...
    ``sys.modules``. ``agents.openai_sdk_v2.llm_client`` is preserved so
    ...
    for _k in list(sys.modules.keys()):
        ...
        if _k == "tools" or _k.startswith("tools."):
            del sys.modules[_k]
        if _k in (
            "agents.openai_sdk_v2.agent",
            "agents.openai_sdk_v2.prompts",
            "agents.openai_sdk_v2.tools",
        ):
            del sys.modules[_k]
...
def mock_openai(monkeypatch):
    ...
    with patch("agents.openai_sdk_v2.llm_client.OpenAI") as m:
        yield m
    _reset_llm_client_cache()
...
class TestLlmClientCaching:
    def test_get_client_returns_cached_instance(self, mock_openai):
        from agents.openai_sdk_v2.llm_client import get_client
        c1 = get_client()
        c2 = get_client()
        assert c1 is c2
...
class TestTransientClassification:
    def test_timeout_is_transient(self):
        ...
        from agents.openai_sdk_v2.llm_client import _is_transient
...
class TestChatRetry:
    def test_returns_completion_on_success(self, mock_openai):
        from agents.openai_sdk_v2.llm_client import chat
...
class TestToolSchema:
    def test_required_tools_present(self):
        from agents.openai_sdk_v2.tools import TOOLS
    ...
    def test_build_tools_returns_expected_count(self):
        ...
        from agents.openai_sdk_v2.tools import build_tools
    ...
    def test_system_prompt_under_length_threshold(self):
        ...
        from agents.openai_sdk_v2.prompts import build_system_prompt
...
class TestToolHandlers:
    def test_analyze_task_returns_json(self):
        from agents.openai_sdk_v2.tools import build_handlers
    ...
    def test_submit_late_window_ships(self):
        """Happy path inside the late-window — submit raises SubmitSignal."""
        from agents.openai_sdk_v2.tools import SubmitSignal, build_handlers
        # Deadline 60s away → inside the late-window (< 5 minutes).
        handlers = build_handlers(
            _make_challenge(), deadline=time.monotonic() + 60,
        )
    ...
    def test_submit_late_window_no_note_still_ships(self, capsys):
        """No-note path inside the late-window — submit no longer
        blocks. SubmitSignal still fires, with one stderr warning."""
        from agents.openai_sdk_v2.tools import SubmitSignal, build_handlers
        handlers = build_handlers(
            _make_challenge(), deadline=time.monotonic() + 60,
        )
    ...
    def test_submit_no_deadline_ships(self):
        """If deadline is None, treat remaining=0 and ship — preserves
        backward compat with no-deadline tests / harness paths."""
        from agents.openai_sdk_v2.tools import SubmitSignal, build_handlers
        handlers = build_handlers(_make_challenge())
        handlers["write_scratchpad"](observation="ok")
        with pytest.raises(SubmitSignal) as excinfo:
            handlers["submit"](
                code=VALID_CODE, name="m", motivation="no deadline",
            )
        assert excinfo.value.code == VALID_CODE

    def test_submit_early_window_stashes_best_so_far(self):
        """Outside the late-window, submit returns a 'stashed' string
        and writes best_so_far into state — does NOT raise SubmitSignal."""
        from agents.openai_sdk_v2.tools import (
            EARLY_SUBMIT_GATE_SECONDS, SubmitSignal, build_handlers,
        )
        # Deadline far in the future → before the late-window.
        handlers = build_handlers(
            _make_challenge(),
            deadline=time.monotonic() + EARLY_SUBMIT_GATE_SECONDS + 600,
        )
        handlers["write_scratchpad"](observation="ok")
        ...
        # The state holder now has best_so_far with the candidate.
...
class TestReadMySubmissions:
    ...
    def _ship(self, handlers, code, name, motivation, candidate_id=""):
        from agents.openai_sdk_v2.tools import SubmitSignal
        handlers["write_scratchpad"](observation="ok")
    ...
    def test_score_updated_when_previous_results_arrive(self):
        ...
        from agents.openai_sdk_v2.tools import build_handlers
        from agents.openai_sdk_v2.core.history import (
...
class TestFileTools:
    ...
    def test_enforces_size_cap(self, tmp_path):
        from agents.openai_sdk_v2.tools import build_handlers, FILE_MAX_BYTES
        handlers = build_handlers(
            _make_challenge(), scratch_dir=str(tmp_path),
        )
...
class TestDesignArchitecture:
    def test_submit_on_first_turn(self, mock_openai):
        ...
        from agents.openai_sdk_v2 import agent
    ...
    def test_early_submit_then_no_resubmit_ships_stashed(self, mock_openai):
        """Long budget: LLM submits early (stashes), then next turn has
        no tool calls. The recovery branch in agent.py must ship the
        stashed best_so_far rather than falling through to fallback."""
        from agents.openai_sdk_v2 import agent

        # Round 1: submit tool call (stashes because budget > 300s).
        # Round 2: plain text, no tool calls → loop breaks → recovery.
        tc = _make_tool_call("submit", {
            "code": VALID_CODE,
            "name": "stashed_pick",
            "motivation": "first attempt",
        })
        first = _make_completion(tool_calls=[tc])
        second = _make_completion(content="thinking, but no further code")
        mock_openai.return_value.chat.completions.create.side_effect = [
            first, second,
        ]

        ch = _make_challenge(with_flops=False)
        # Budget large enough that the submit lands outside the
        # late-window (> EARLY_SUBMIT_GATE_SECONDS).
...
class TestExtractCodeBlock:
    def test_finds_python_fence(self):
        from agents.openai_sdk_v2.agent import _extract_code_block
        text = "Here is code:\n```python\nprint('hi')\n```\nThanks."
