...
def build_system_prompt(challenge: dict, bucket: str | None = None) -> str:
    ...
    parts.append(
        "## Core tools\n\n"
        "These cover a clean run end-to-end. Reach for the advanced "
        "tools below only when one of these isn't enough.\n\n"
        "- `analyze_task` — task spec, constraints, objectives, FLOPs "
        "budget. Read this first if the task is unfamiliar.\n"
        "- `read_scratchpad` — your prior-round notes (hypotheses, "
        "dead ends, observations) and submission history with scores. "
        "Cheap; check it at the start of each round.\n"
        "- `list_frontier` / `get_frontier_member` — current best "
        "models on this task. Frontier is data, not a target — read it "
        "for ideas, then do something different and better.\n"
        "- `sketch_architecture` — draft a `build_model` and probe "
        "it; reports FLOPs + per-layer trace + output-shape check. "
        "Cheaper than `validate_code`. Use freely while iterating. "
        "Returns a stable `candidate_id` (cand_<hex>) keyed by the "
        "code's hash; `validate_code` and `submit` both accept that "
        "id so you don't re-paste source.\n"
        "- `size_to_flops` — sweep one scalar knob (hidden_dim, "
        "num_layers, etc.) to land inside the FLOPs gate. Pass code "
        "with `{{SIZE}}` as the placeholder; tool returns the best "
        "(size, flops) pair.\n"
        "- `validate_code` — final pre-submission check. Passes only "
        "if syntax + build_model/build_optimizer + FLOPs gate + "
        "output shape all hold. Required before submit. Accepts a "
        "`candidate_id` in place of `code`.\n"
        "- `write_scratchpad` — record one structured note per call: "
        "`hypothesis` (with optional `candidate_id` link), `dead_end` "
        "+ `reason`, or `observation`. You can also attach the note "
        "inline on `submit(note=...)` to skip a separate call.\n"
        "- `submit` — ship validated code in a single call: "
        "`submit(candidate_id=..., name=..., motivation=..., "
        "note=...)`. The `note` is recorded into the scratchpad "
        "before the loop ends — when `candidate_id` is supplied it "
        "becomes a hypothesis linked to that candidate so next round "
        "you can see what shipped vs. what scored.\n"
        "- `time_remaining` — check the clock when it matters.\n\n"
        "## Advanced tools (optional)\n\n"
        "- `read_my_submissions(n=3)` — full code of your prior "
        "submissions (read_scratchpad shows summaries only).\n"
        "- `link_hypothesis` — late-bind a candidate_id or a verdict "
        "(supported/refuted/inconclusive) to an existing hypothesis.\n"
        "- `trace_architecture` / `check_output_shape` — finer shape "
        "debugging when `sketch_architecture` flags something.\n"
        "- `estimate_flops` / `estimate_layer_flops` — standalone "
        "FLOPs accounting for arbitrary code or layers.\n"
        "- `cognition_wiki_index` / `cognition_wiki_read` — curated "
        "task-specific corpus of architecture-design and training "
        "recipes. Try this before `search_papers`.\n"
        "- `search_papers` — arxiv search.\n"
        "- `query_db` — recent experiments across all miners "
        "(`/experiments/recent`, `/experiments/pareto`, "
        "`/experiments/stats`, POST `/experiments/search`).\n"
        "- `list_files` / `read_file` / `write_file` / `search_files` "
        "— cross-round persistent text notes (design.md, etc.).\n"
        "- `define_macro` / `run_macro` / `list_macros` — save and "
        "replay a reusable tool sequence; `submit`, `define_macro`, "
        "and `run_macro` are not allowed inside a macro."
    )

    parts.append(
        "## Example trajectory\n\n"
        "A clean run looks like this — five or six tool calls, ship.\n\n"
        "1. `analyze_task` — confirm task params, output shape, "
        "constraints.\n"
        "2. `list_frontier` — see what's been tried.\n"
        "3. `sketch_architecture(code=...)` → returns "
        "`candidate_id: cand_a3f24c1d`.\n"
        "4. `size_to_flops(code=..., knob=\"hidden_dim\")` — land "
        "the FLOPs gate (skip if the sketch already lands).\n"
        "5. `validate_code(candidate_id=\"cand_a3f24c1d\")` → `ok`.\n"
        "6. `submit(candidate_id=\"cand_a3f24c1d\", "
        "name=\"depthwise_sep_v1\", motivation=\"...\", "
        "note=\"depthwise-sep at hidden=64 — same RF, lower FLOPs\")` "
        "— shipped.\n"
    )
