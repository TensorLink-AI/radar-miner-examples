...
class TestCodeValidity:
    def test_all_buckets_produce_valid_code(self):
        ...
        for bucket, cfg in _SCALING.items():
...
class TestScaling:
    def test_all_buckets_have_scaling(self):
        for bucket in SIZE_BUCKETS:
            assert bucket in _SCALING, f"Missing scaling config for {bucket}"

    def test_d_model_increases_with_bucket(self):
        order = ["tiny", "small", "medium_small", "medium", "large"]
        dims = [_SCALING[b]["d_model"] for b in order]
        for i in range(len(dims) - 1):
            assert dims[i] <= dims[i + 1], \
                f"d_model should increase: {order[i]}={dims[i]} > {order[i+1]}={dims[i+1]}"

    def test_n_layers_increases_with_bucket(self):
        order = ["tiny", "small", "medium_small", "medium", "large"]
        layers = [_SCALING[b]["n_layers"] for b in order]
        for i in range(len(layers) - 1):
            assert layers[i] <= layers[i + 1], \
                f"n_layers should increase: {order[i]}={layers[i]} > {order[i+1]}={layers[i+1]}"

    def test_patch_size_consistent(self):
        """All buckets use the same patch size for simplicity."""
        sizes = {_SCALING[b]["patch_size"] for b in _SCALING}
        assert len(sizes) == 1, f"Expected uniform patch_size, got {sizes}"
...
class TestChallengeParsing:
    ...
    def test_center_flops_is_midpoint(self):
        """The agent should target the center of the bucket."""
        client = MockClient()
        ch = _make_challenge("medium")
        result = _run(ch, client)
        # medium bucket: (10M, 50M), center = 30M
